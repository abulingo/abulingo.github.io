...
navBarra.innerHTML=`
    <header class="fixed top-0 left-0 w-full bg-white/80 backdrop-blur-lg py-3 z-50 border-b border-blue-100">
        <div class="container mx-auto flex justify-between items-center px-6">
            ...
            <nav class="hidden md:flex items-center space-x-2">
                ...
                <a href="traductor.html" class="nav-link active">Traductor</a>
    ...
    <div id="mobile-menu" class="hidden md:hidden fixed top-20 right-4 w-56 bg-white/90 backdrop-blur-lg p-4 rounded-lg shadow-xl z-40 space-y-2 flex flex-col">
        ...
        <a href="traductor.html" class="nav-link text-center active">Traductor</a>
        <a href="soporte.html" class="nav-link text-center">Soporte</a>
    </div>
`
// Ajustar la posición top del mobile-menu dinámicamente para que quede justo debajo del header
const header = document.querySelector('header');
if (header) {
    const headerHeight = header.offsetHeight;
    const mobileMenu = document.getElementById('mobile-menu');
    if (mobileMenu) {
        mobileMenu.style.top = `${headerHeight}px`; // Añadí +8px para un poco más de espacio debajo
    }
}
